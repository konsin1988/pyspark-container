ATTACH TABLE _ UUID '15e53b5a-0282-4f8d-9a3b-d9d2f6f1ba81'
(
    ...
    `ProfileEvent_ReadTasksWithAppliedMutationsOnFly` UInt64 COMMENT 'Total number of parts for which there was any mutation applied on fly',
    `ProfileEvent_MutationsAppliedOnFlyInAllReadTasks` UInt64 COMMENT 'The sum of number of applied mutations on-fly for part among all read parts',
    ...
    `ProfileEvent_IcebergPartitionPrunnedFiles` UInt64 COMMENT 'Number of skipped files during Iceberg partition pruning',
    ...
    `ProfileEvent_IcebergMinMaxIndexPrunnedFiles` UInt64 COMMENT 'Number of skipped files by using MinMax index in Iceberg',
    ...
    `ProfileEvent_DistrCacheIgnoredBytesWhileWaitingProfileEvents` UInt64 COMMENT 'Distributed Cache read buffer event. Ignored bytes while waiting for profile events in distributed cache',
    ...
    `ProfileEvent_DistrCachePacketsBytes` UInt64 COMMENT 'Distributed Cache client event. The number of bytes in Data packets which were not ignored',
    ...
    `ProfileEvent_DistrCacheUnusedPacketsBytes` UInt64 COMMENT 'Distributed Cache client event. The number of bytes in Data packets which were ignored',
    ...
    `ProfileEvent_DistrCacheReadBytesFromCache` UInt64 COMMENT 'Distributed Cache read buffer event. Bytes read from distributed cache',
    ...
    `ProfileEvent_ConcurrencyControlSlotsAcquired` UInt64 COMMENT 'Total number of CPU slot acquired',
    ...
    `CurrentMetric_IcebergMetadataFilesCacheSize` Int64 COMMENT 'Size of the iceberg metadata cache in bytes',
    `CurrentMetric_VectorSimilarityIndexCacheSize` Int64 COMMENT 'Size of the vector similarity index cache in bytes',
